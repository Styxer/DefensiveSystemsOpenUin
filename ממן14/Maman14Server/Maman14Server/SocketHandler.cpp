...
bool SocketHandler::receive(boost::asio::ip::tcp::socket& socket, uint8_t(&buffer)[PACKET_SIZE])
{
	bool result = false;
	try 
	{
...
/// <returns>true is sent succesfully. false otherwise</returns>
bool SocketHandler::send(boost::asio::ip::tcp::socket& socket, const uint8_t(&buffer)[PACKET_SIZE])
{
	bool result = false;
	try
	{
