#include "../Headers/RSAWrapper.h"


RSAPublicWrapper::RSAPublicWrapper(const uint8_t* key, unsigned int length)
{
...
uint8_t* RSAPublicWrapper::getPublicKey(uint8_t* keyout, unsigned int length) const
{
...
RSAPrivateWrapper::RSAPrivateWrapper(const uint8_t* key, unsigned int length)
{
	CryptoPP::StringSource ss(reinterpret_cast<const CryptoPP::byte*>(key), length, true);
	_privateKey.Load(ss);
}
...
uint8_t* RSAPrivateWrapper::getPrivateKey(uint8_t* keyout, unsigned int length) const
{
	CryptoPP::ArraySink as(reinterpret_cast<CryptoPP::byte*>(keyout), length);
	_privateKey.Save(as);
	return keyout;
...
uint8_t* RSAPrivateWrapper::getPublicKey(uint8_t* keyout, unsigned int length) const
{
	CryptoPP::RSAFunction publicKey(_privateKey);
	CryptoPP::ArraySink as(reinterpret_cast<CryptoPP::byte*>(keyout), length);
	publicKey.Save(as);
