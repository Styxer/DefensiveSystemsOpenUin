...
uint8_t* AESWrapper::GenerateKey(uint8_t* buffer, unsigned int length)
{
...
AESWrapper::AESWrapper(const uint8_t* key, unsigned int length)
{
...
void AESWrapper::setKey(const std::string& newKey)
{
	std::copy(newKey.begin(), newKey.end(), this->_key);
}
...
std::string AESWrapper::encrypt(const uint8_t* plain, unsigned int length)
{
...
std::string AESWrapper::decrypt(const uint8_t* cipher, unsigned int length)
{
	CryptoPP::byte iv[CryptoPP::AES::BLOCKSIZE] = { 0 };	// for practical use iv should never be a fixed value!
